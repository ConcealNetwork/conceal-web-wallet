...
if (typeof window !== 'undefined') {
  window.config = myGlobal.config;
}

function logDebugMsg(...data: any[]) {
  if (myGlobal.config.debug) {
    if (data.length > 1) {
...
declare global {
  interface Window {
    config: typeof myGlobal.config;
  }
}

export type ConfigType = typeof myGlobal.config;

export default myGlobal.config;
